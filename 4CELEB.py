...
celeba_dataset = datasets.ImageFolder(root=".\celeba_dataset", transform=transform)
# Create DataLoader (batch of 32 images)

subset_size = 100000  # Number of samples to use
celeba_subset = Subset(celeba_dataset, range(subset_size))
dataloader = torch.utils.data.DataLoader(celeba_subset, batch_size=32, shuffle=True)
...
class Generator(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Unflatten(1, (128, 8, 8)),  # Reshape to (128, 8, 8)
            nn.Upsample(scale_factor=2),  # Upsample to (128, 16, 16)
            nn.Conv2d(128, 128, kernel_size=3, padding=1),
            nn.BatchNorm2d(128, momentum=0.78),
            nn.ReLU(),
            nn.Upsample(scale_factor=2),  # Upsample to (128, 32, 32)
            nn.Conv2d(128, 64, kernel_size=3, padding=1),
            nn.BatchNorm2d(64, momentum=0.78),
            nn.ReLU(),
            nn.Upsample(scale_factor=2),  # **Extra Upsample to (64, 64)**
            nn.Conv2d(64, 32, kernel_size=3, padding=1),
            nn.BatchNorm2d(32, momentum=0.78),
            nn.ReLU(),
            nn.Conv2d(32, 3, kernel_size=3, padding=1),  # Final Conv layer
            nn.Tanh()
        )
...
class Discriminator2(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(3, 48, kernel_size=4, stride=2, padding=1),  # Different kernel size and channels
            nn.LeakyReLU(0.2),
            ...
            nn.LeakyReLU(0.2),
            nn.Dropout(0.3),
            nn.Conv2d(192, 384, kernel_size=4, stride=1, padding=1),
            nn.BatchNorm2d(384, momentum=0.8),
            nn.LeakyReLU(0.2),
            nn.Flatten(),
            nn.Linear(384 * 7 * 7, 1),  # Adjusted for different channel count
            nn.Sigmoid()
        )

    def forward(self, img):
      out = self.model[:-3](img)  # Extract features before flattening
      #print("Feature map shape before flattening:", out.shape)  # Debugging
      out = out.view(out.size(0), -1)  # Flatten
      validity = self.model[-3:](out)  # Pass through FC layer
      return validity
...
class Discriminator3(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Flatten(),
            nn.Linear(512 * 7 * 7, 1),  # Adjusted for feature map size
            nn.Sigmoid()
        )
...
class Discriminator4(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Flatten(),
            nn.Linear(1024 * 8 * 8, 1),  # Adjusted for feature map size
            nn.Sigmoid()
        )
...
class Discriminator5(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.LeakyReLU(0.1),
            nn.Flatten(),
            nn.Linear(512 * 8 * 8, 1),  # Adjusted for feature map size
            nn.Sigmoid()
        )
...
for epoch in range(num_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        g_loss.backward()
        optimizer_G.step()
        generator_losses.append(g_loss / len(dataloader))
        discriminator_losses.append(d_loss_total / len(dataloader))
